...
import { joinRoom, onError, onPlayers, onState, sendAction } from "./socketClient";
import type { ConnectedPlayer } from "./socketClient";
...
export function useRoomOnlineGame(args: {
  ...
  return useMemo(
    () => ({ state, dispatch, error, serverIsMaster, isHost, myTeamId, connectedPlayers }),
    [state, dispatch, error, serverIsMaster, isHost, myTeamId, connectedPlayers],
  );
}
